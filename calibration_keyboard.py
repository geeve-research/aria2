   ...
   def on_image_received(self, image: np.array, record: ImageDataRecord) -> None:
       if record.camera_id == aria.CameraId.EyeTrack:
           ...
           try:
               ...
               raw_point = (float(preds[0][0]), float(preds[0][1]))
               calibrated_point = (float(calibrated_preds[0]), float(calibrated_preds[1]))
...
class EyeTrackingVisualizer:
   ...
   def draw_gaze(self, screen, observer, color=(0, 255, 0)):
       if observer.value_mapping:
           # Draw raw gaze point
           raw_x = int((observer.value_mapping["raw_x"] + 0.5) * self.screen_width)
           raw_y = int((observer.value_mapping["raw_y"] + 0.5) * self.screen_height)
           
           cv2.circle(screen, (raw_x, raw_y), 10, (0, 0, 255), 2)  # Red for raw
           ...
           cal_x = int((observer.value_mapping["gaze_x"] + 0.5) * self.screen_width)
           cal_y = int((observer.value_mapping["gaze_y"] + 0.5) * self.screen_height)
           ...
           # Draw gaze trail
           points = [(int((x[1][0] + 0.5) * self.screen_width), 
                     int((x[1][1] + 0.5) * self.screen_height)) 
                    for x in observer.gaze_history]
           if len(points) > 1:
   ...
   def run(self, observer):
    ...
    while True:
        if current_mode == "main_menu":
            ...
            elif key == ord('3'):  # Add this section
        ...
        elif current_mode == "grid_test":  # Add this section
